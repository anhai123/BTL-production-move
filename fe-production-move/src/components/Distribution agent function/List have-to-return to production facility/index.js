...
const ListHaveToReturnToProductionFacilityProduct = () => {
  ...
  const updateStatusProduct = () => {
    setLoading(true);
    DistributionAgentServices.putSttProductWhenReturnToProductFacility(
      selectedRowKeys
    ).then(
      (response) => {
        DistributionAgentServices.getNeedToDeliverToProductFacilityProduct().then(
          (response) => {
  ...
  return (
    <>
      ...
      <Table
        ...
        dataSource={data}
        locale={{
          emptyText: "Không có sản phẩm nào cần phải trả về cơ sở sản xuất",
        }}
      />
    </>
  );
};
export default ListHaveToReturnToProductionFacilityProduct;
