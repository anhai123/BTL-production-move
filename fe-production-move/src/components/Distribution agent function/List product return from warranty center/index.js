...
const ReceivingProductFromWarrantyCenter = () => {
  ...
  const updateStatusProduct = () => {
    setLoading(true);
    DistributionAgentServices.putSttIncomingToShippedToDistributionAgents(
      selectedRowKeys
    ).then(
      (response) => {
  ...
  return (
    <>
      ...
      <Table
        ...
        locale={{
          emptyText:
            "Không có sản phẩm nào được gửi đến đại lý từ trung tâm bảo hành",
        }}
      />
    </>
  );
};
export default ReceivingProductFromWarrantyCenter;
