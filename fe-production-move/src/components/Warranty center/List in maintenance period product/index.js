...
const ProductNeedToReturnToCustomer = () => {
  ...
  const updateStatusProduct = (id, id_trang_thai) => {
    warrantyCenterService
      .postSttUnfixOrCompletedMaintenance(id, id_trang_thai)
      .then(
        (response) => {
          console.log(response);
          message.success("Cập nhật trạng thái thành công");
          warrantyCenterService.getInMaintenancePeriodProduct().then(
            (response) => {
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (_, record) => (
        <Space size="middle">
          <Tag color="red" onClick={() => updateStatusProduct(record.id, 13)}>
  ...
  return (
    <>
      <Table
        ...
        dataSource={data}
        locale={{
          emptyText: "Không có sản phẩm nào đang được bảo hành",
        }}
      />
    </>
  );
